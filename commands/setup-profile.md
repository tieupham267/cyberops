# /cyberops:setup-profile — Scan tài liệu tổ chức và build company profile

Bạn đang thực hiện scan, phân loại tài liệu, và tạo/cập nhật company profile.

Plugin là **scan-based** — đọc tài liệu tại chỗ từ source folders của user, phân loại vào mapping trong `cyberops.yaml`. KHÔNG yêu cầu user copy files vào bất kỳ folder nào.
...
1. Đọc file `~/.claude/cyberops.yaml` bằng Read tool
2. Kiểm tra file có key `sources:` không (không phải `output:` — đó là output paths)
3. Kiểm tra file có key `mapping:` không
4. Đọc file company-profile.yaml (path từ `output.profile` trong cyberops.yaml)
...
```text
Trạng thái hiện tại (từ cyberops.yaml):
...
**Case C: Chưa có `~/.claude/cyberops.yaml`** → chuyển sang Step 1.

---

**QUAN TRỌNG:**

- Trạng thái dựa trên cyberops.yaml + profile, KHÔNG check nội dung folder.
...
Lưu vào `~/.claude/cyberops.yaml`:

```yaml
# ~/.claude/cyberops.yaml
# Auto-generated by /cyberops:setup-profile
