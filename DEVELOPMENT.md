...
SECOPS_PROFILE=dev claude --plugin-dir ./secops-plugin

# Standard (mặc định) — tất cả checks
claude --plugin-dir ./secops-plugin

# Strict — tất cả checks + block warnings (cho audit/production)
SECOPS_PROFILE=strict claude --plugin-dir ./secops-plugin
```
...
Mở VS Code → mở folder `secops` → chạy `claude --plugin-dir ./secops-plugin` trong terminal → nhờ Claude chỉnh sửa/thêm files → `/reload-plugins` để test → commit + push khi hài lòng.
...
claude --plugin-dir ./secops-plugin/secops
