...
| 8 | Cài Memvid — Semantic Search *(tùy chọn)* | 15 phút |
| 9 | Verify & test | 5 phút |
| | **Tổng** | **~85 phút** |
...
## Bước 8: Cài Memvid — Semantic Search cho tài liệu team *(tùy chọn)*

Khi working directory có **hàng trăm đến hàng ngàn file** đa format (PDF, DOCX, XLSX, logs...), Grep/Glob không đủ hiệu quả: không search được trong PDF/DOCX, không hiểu ngữ nghĩa, không rank relevance. [Memvid](https://github.com/memvid/memvid) giải quyết vấn đề này bằng cách index toàn bộ tài liệu vào **một file portable**, hỗ trợ semantic search + full-text search.

> **Không cài memvid thì plugin vẫn hoạt động bình thường** — tất cả agents fallback về Grep/Glob trên working directory.

### Khi nào cần?

- Folder team cybersec có hàng ngàn file: incident reports, scan results, policies, threat intel, audit evidence...
- Tài liệu đa format: PDF, DOCX, XLSX — Grep không đọc được
- Cần search đa ngôn ngữ: query tiếng Việt tìm được tài liệu tiếng Anh và ngược lại
- Cần tiết kiệm token: memvid trả top kết quả thay vì Claude Code phải mò qua hàng trăm file
...
# Hoặc CLI (xem hướng dẫn tại https://github.com/memvid/memvid)
```

### Index lần đầu

Tạo index từ folder tài liệu team:

```bash
memvid index "D:\CyberSec-Team" --output "D:\CyberSec-Team\memory.mv2e" --encrypt
```

> Index lần đầu có thể mất vài phút đến vài chục phút tùy số lượng file. File `.mv2e` được encrypt — phù hợp cho dữ liệu nhạy cảm.

### Cấu hình cho plugin

Thêm block `memvid` vào `~/.claude/cyberops.yaml`:

```yaml
# ~/.claude/cyberops.yaml
sources:
  - path: "D:\\CyberSec-Team"
...
```bash
# Test search
memvid search "D:\CyberSec-Team\memory.mv2e" "incident privilege escalation"
```

Nếu trả về kết quả → cấu hình thành công. Orchestrator sẽ tự động dùng memvid khi cần semantic search.

### Vận hành: cập nhật index khi tài liệu thay đổi

Memvid dùng **append-only smart frames** — chỉ cần thêm files mới/thay đổi, không rebuild toàn bộ.

**Cách 1 — Chạy tay** *(khi nhớ)*

```bash
memvid index "D:\CyberSec-Team" --output "D:\CyberSec-Team\memory.mv2e" --update
```

**Cách 2 — Schedule tự động** *(recommended)*

```bash
# Windows Task Scheduler (chạy hàng ngày lúc 2h sáng):
schtasks /create /tn "Memvid Index Update" /tr "memvid index D:\CyberSec-Team --output D:\CyberSec-Team\memory.mv2e --update" /sc daily /st 02:00

# Linux/macOS cron:
# 0 2 * * * memvid index /path/to/team --output /path/to/memory.mv2e --update
```

**Cách 3 — Git hook** *(nếu tài liệu trong git)*

Thêm post-merge hook để tự update index khi pull changes.

> **Lưu ý**: Việc giữ index up-to-date là trách nhiệm ngoài plugin. Plugin chỉ đọc file `.mv2e` theo path trong config — không tự quản lý index.

### Khi nào orchestrator dùng memvid vs Grep

| Tình huống | Memvid | Grep/Glob |
| --- | --- | --- |
| Tìm theo nghĩa (semantic) | ✅ Ưu tiên | ❌ Không hỗ trợ |
| Tìm trong PDF/DOCX/XLSX | ✅ Ưu tiên | ❌ Không hỗ trợ |
| Tìm cross-language (VN ↔ EN) | ✅ Ưu tiên | ❌ Không hỗ trợ |
| Tìm exact string (CVE ID, IP, hash) | Dùng được | ✅ Ưu tiên (nhanh hơn) |
| Tìm trong file text/Markdown/YAML | Dùng được | ✅ Ưu tiên (nhanh hơn) |

Chi tiết logic: xem [agents/orchestrator.md](agents/orchestrator.md) → section "Document Search".
