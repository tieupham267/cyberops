...
**ALWAYS read `context/company-profile.yaml`** (từ `$SECOPS_HOME` hoặc working directory) for company tech stack and payment infrastructure. Nếu file không tồn tại → hướng dẫn user chạy `/secops:setup-profile`.
